...
def save_noun_cross_attenton(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0,noun_idx:List[int]=[],image_id=None):
    ...
    torch.save(save_ts,f'./outputs1/attn_db/{image_id}.pt')
...
def save_multi_scale_attention_map(attention_store: AttentionStore,tag_id=None,idx=None):
    if not osp.exists(f'./outputs1/attn_db/{tag_id}/cross16_{idx}.pt'):
        cross_16 = aggregate_attention(attention_store,16,['down','up'],is_cross=True,select=0)
        save_torch_tensor(cross_16,f'./outputs1/attn_db/{tag_id}/cross16_{idx}.pt')

    if not osp.exists(f'./outputs1/attn_db/{tag_id}/cross32_{idx}.pt'):
        cross_32 = aggregate_attention(attention_store,32,['down','up'],is_cross=True,select=0)
        save_torch_tensor(cross_32,f'./outputs1/attn_db/{tag_id}/cross32_{idx}.pt')

    if not osp.exists(f'./outputs1/attn_db/{tag_id}/cross64_{idx}.pt'):
        cross_64 = aggregate_attention(attention_store,64,['down','up'],is_cross=True,select=0)
        save_torch_tensor(cross_64,f'./outputs1/attn_db/{tag_id}/cross64_{idx}.pt')

    if not osp.exists(f'./outputs1/attn_db/{tag_id}/self_16.pt'):
        self_16 = aggregate_attention(attention_store,16,['down','up'],is_cross=False,select=0)
        save_torch_tensor(self_16,f'./outputs1/attn_db/{tag_id}/self_16.pt')

    if not osp.exists(f'./outputs1/attn_db/{tag_id}/self_32.pt'):
        self_32 = aggregate_attention(attention_store,32,['down','up'],is_cross=False,select=0)
        save_torch_tensor(self_32,f'./outputs1/attn_db/{tag_id}/self_32.pt')
#
    if not osp.exists(f'./outputs1/attn_db/{tag_id}/self_64.pt'):
        self_64 = aggregate_attention(attention_store,64,['down','up'],is_cross=False,select=0)
        save_torch_tensor(self_64,f'./outputs1/attn_db/{tag_id}/self_64.pt')
...
def save_visualize_cross_attn_map(cross_attn,decoder,tokens,file_name):
    ...
    view_images(np.concatenate(vis_images, axis=1)).save(f'./outputs1/nulltext_png_multi-step/{file_name}.png')
...
if __name__=='__main__':
    ...
    data = json.load(open("./data/rrsisd.json"))
    data = data["test"]
    ...
    for idx,i in tqdm(enumerate(range(len(data))),total=len(data)):
        ...
        image_id = data[i]['iid']
        tag_id = image_id
        if osp.exists(f'./outputs1/attn_db/{tag_id}/'):
            pass
        else:
            os.makedirs(f'./outputs1/attn_db/{tag_id}')

        image_path = osp.join(
            "./data/images",
            "{:05d}.jpg".format(int(image_id)),
        )
        prompt = data[i]['refs'][0]
